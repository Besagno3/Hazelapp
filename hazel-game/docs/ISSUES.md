...
| #63 | 🟡 | Low    | Wandering NPCs/enemies (`lib/wander.ts` + `WorldCanvas`) use a simple leashed random walk: no actor↔actor avoidance (two wanderers can briefly overlap), and ambient speech bubbles fade on real time so one that spawns just before an overlay opens keeps fading underneath it (cosmetic; spawning is paused). Wander tuning (speed/leash) is hard-coded in `WorldCanvas`, not in the content/tuning layer. Revisit if the overworld ever feels crowded or jittery. |
