...
| #67 | 🟡 | Med    | Wave 0.4: the generate-questions topic whitelist + persona lines moved to `supabase/functions/_shared/topics.ts`, shared by the edge function (relative `.ts` import, bundled on deploy) and the app (`src/content/topicPrompts.ts` re-export). Drift is now BOTH a compile error (`_topicSetsMatch` in topicPrompts.ts proves `Topic` ≡ `TopicId`) and a test failure (`topicPrompts.test.ts`). Adding a topic no longer touches function internals. ⚠️ **Deploy — CLI only:** `supabase functions deploy generate-questions` (from `hazel-game/`) bundles the `_shared` module. The function is **no longer a single file** — the old "paste index.ts into the Supabase dashboard editor" workflow (how #57 was redeployed) will now boot-fail with module-not-found and 500 every question fetch. Behavior is otherwise identical (same 7 topics, same prompt text). |
