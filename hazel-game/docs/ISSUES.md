...
| #64 | 🟢 | Low    | XP-to-next-level was a flat 100 for every level, so leveling never got harder. Now the per-level cost **grows by a tapering, deterministic-random percentage** (`lib/level.ts`): ~60% growth early, decaying toward ~8% at high levels, with a level-seeded wobble. Level is still derived from total XP (nothing stored), so the growth is seeded by the level index to stay refresh-proof. Side effect: the same total XP now maps to a **lower displayed level** past level 2 (e.g. 250 XP was level 3, is now level 2) — expected, no data migration. `LevelBadge` needed no change (it already renders the dynamic `needed`). |
