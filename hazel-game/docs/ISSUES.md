...
| #67 | 🟡 | Med    | Wave 0.4: the generate-questions topic whitelist + persona lines moved to `supabase/functions/_shared/topics.ts`, shared by the edge function (relative `.ts` import, bundled on deploy) and the app (`src/content/topicPrompts.ts` re-export). `topicPrompts.test.ts` locks the shared list to the game's `ALL_TOPICS` — drift now fails the suite. Adding a topic no longer touches function internals, but **still requires** `supabase functions deploy generate-questions`. ⚠️ Deploy: redeploy once so the function picks up the `_shared` module layout (behavior identical — same 7 topics, same prompt text). |
